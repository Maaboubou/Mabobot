...
| 工作目录 | 本聊天空间 | 项目工作目录 |
| 文件范围 | 本聊天空间只读或可读写，必要运行组件和 Skills 只读 | 使用最大访问模式，仍受操作系统账号权限约束 |
| shell 网络 | 按有效策略禁止或经已验证代理访问公共互联网 | 不受该隔离禁网策略限制 |
| 提权 | 不允许申请突破隔离范围 | 按请求与自动审批策略处理 |

群聊不能获得管理员私聊模式：管理接口检查一次，运行时再次限制。隔离模式以默认拒绝为基础，仅开放工作区和必要运行资源，并检查目录中的符号链接等越界风险。

权限默认和本聊天差异由统一解析器合成，文件读写与 shell 公网组合为四个有限 Profile。
现有安装迁移保留公网关闭；管理员显式允许公网后，必须同时验证网络代理与私网拦截。
在线研究工具另行控制搜索、浏览器和审核下载；插件授权独立。配置保存和运行时生效分别展示。
详见 [权限中心使用与验收](CODEX_PERMISSION_CENTER_OPERATIONS.md)，包括自动审阅的当前限制。

公开文件还可通过 `wx_browser.download` 保存到本次请求输出目录。该工具使用宿主
的公网 IP 校验、固定 IP 代理和重定向检查，可处理环境中的 Fake-IP DNS，默认限制
单文件 256 MiB，并按管理员定义的微信内容规则独立审核。网页工具能打开视频链接
不代表已保存视频；只有审核通过并返回实际下载路径才可作为附件列入交付清单。
Codex 助手在最终附件和发送副本处也执行审核；拒绝、不确定或无法解析时暂停发送。
