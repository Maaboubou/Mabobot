...
2026-09-25 备份与恢复流程调整：顶部只保留创建、导入、刷新入口；创建范围移到独立编辑区，恢复使用影响确认、准备计划、重启应用三个明确阶段。记录动作改为文字按钮，删除默认使用中性色，说明文字调整为 12px、正文与控件为 13px；备份组件只使用语义令牌。编辑器刷新不重建，恢复面板主动定位并管理焦点，待恢复计划可继续或取消，任务结果与错误可持续查看。列表和手机布局沿用连续表面。资源缓存串 `20260925-backup-flow`。验证脚本：`tests/browser/backup_flow.py`（390/768/1024/1440px × 明暗，范围保留、重复提交、取消计划、校验、上传、重试、焦点与溢出），`tests/browser/backup_compatibility.py`（390/1440px 旧格式动作与 Profile 排除）。本次预期视觉变化限定在备份组件：工具栏、创建与恢复编辑区、记录操作和状态反馈；截图位于 `/tmp/backup-flow-shots/`，使用模拟数据。

本批验证结果：备份服务、启动、Profile、恢复计划生命周期与 Web 契约共 92 passed（382 subtests）；备份交互八组及兼容性两组通过。全站样式隔离检查将当前 CSS 与移除本次备份专用样式块的 CSS 分别采集 104 视图，产物为 `/tmp/backup-isolation-{base,current}.json`：95 视图一致，8 组概览差异仅为既有脉搏动画采样，另 1 组模型页导航颜色过渡延长等待后复验一致（`/tmp/backup-llm-{base,current}.json`）。此检查验证专用样式不外溢，不代表修改前后备份页视觉零差异；备份页的有意视觉变化以八组截图为证据。
