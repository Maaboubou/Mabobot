...
执行顺序和 Manifest 监听器声明仍为全局级；执行顺序页编辑全局触发设置。插件业务字段默认支持聊天覆盖，统一的 `get_config()` 自动读取当前聊天的有效值，接入方式见第 2.3 节。聊天页通过统一插件分配管理普通插件权限、推送权限，以及群聊是否要求 @；独立配置不授予权限，推送规则见第 2.2 节。
...
凡是允许用户从页面修改的触发条件，必须先在 `config_schema` 声明，并使用 `group: "trigger"`。业务字段通过 `get_config()` 读取同一个字段，平台自动选择当前聊天的有效值。不要在启动时缓存业务参数，也不要把触发词硬编码在处理器中。
...
`config_schema` 的可编辑业务字段默认允许聊天覆盖，无需额外声明。只有共享进程资源显式标记 `scope: "global"`。敏感字段允许覆盖但不回显、不进入复用模板。提供合法的 `default`，默认配置值统一写在 `config` 中。

- 继承顺序为字段内置默认 → 插件全局值 → 当前聊天的显式覆盖；数组和对象按字段整体替换。
- `register(event_bus, subscribe, context)` 中的 `context.config.resolve(chat_id=event.context.get("chat_id"))` 返回当前插件可覆盖字段的有效值字典。也可直接用 `get_config()`；消息、托管任务和计时器自动沿用本次执行的配置快照。不要把聊天配置存进共享插件实例或写回默认配置。
- `chat_id` 是 EventBus 注入的 `WeChatUser.id`，不是聊天名称、微信 ID 或权限行 ID。解析配置不会授予普通权限或 `#push` 权限。
- 聊天配置由平台主数据库保存，与授权记录独立；复用模板保存完整非敏感可覆盖字段快照，导入时复制，后续修改模板不联动聊天。
...
- 业务参数统一通过 `get_config()` 获取当前聊天有效值，也可用 `context.config.resolve(chat_id=...)` 批量读取。聊天覆盖由平台数据库管理，不另存到插件文件或存储目录。
...
- 业务字段默认支持聊天覆盖，处理器及后台任务读取正确的目标聊天快照；覆盖继承、重置、跨聊天隔离、授权独立和模板复制语义均需验证，详见每聊天配置开发指南。
